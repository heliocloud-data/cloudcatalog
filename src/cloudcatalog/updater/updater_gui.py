...
def updater_gui():
    ...
    def on_submit():
        ...
        try:
            update_catalog_from_json(json_path=json_path_val,
    ...
    default_json_updates = "catalog_stub.json" if os.path.isfile("catalog_stub.json") else ""
    ...
    # Layout
    ttk.Label(root, text="Original Catalog (json_path):").grid(row=0, column=0, sticky="e")
    ttk.Entry(root, textvariable=json_path_var, width=40).grid(row=0, column=1)
    ttk.Button(root, text="Browse", command=lambda: browse_file(json_path_var)).grid(row=0, column=2)

    ttk.Label(root, text="Updates File (json_updates):").grid(row=1, column=0, sticky="e")
    ttk.Entry(root, textvariable=json_updates_var, width=40).grid(row=1, column=1)
    ttk.Button(root, text="Browse", command=lambda: browse_file(json_updates_var)).grid(row=1, column=2)

    ttk.Label(root, text="Collections Filter (optional):").grid(row=2, column=0, sticky="e")
    ttk.Entry(root, textvariable=filter_var, width=40).grid(row=2, column=1)

    ttk.Checkbutton(root, text="Debug mode", variable=debug_var).grid(row=3, column=1, sticky="w")

    ttk.Button(root, text="Update Catalog", command=on_submit).grid(row=4, column=1, pady=10)
