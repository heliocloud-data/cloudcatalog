"""
Validator for new entries to the CloudCatalog.
Returns an error for catalog metadata that doesn't match the spec.

Find the spec here:
https://gitlab.smce.nasa.gov/heliocloud/scregistry/-/blob/develop/docs/cloudcatalog-spec.md?ref_type=heads
...
def validator(catalogname):
    ...
    if "name" not in topcatalog:
        print(
            'No entry for "name" provided! Collection name is a required entry! '
            "Name should be a descriptive "
            "title for the collection of datasets. "
            "Should be the same as provided to GlobalDataRegistry.json."
        )

    if "endpoint" not in topcatalog:
        print(
            'No entry for "endpoint" provided! '
            "Collection endpoint is a required entry!"
        )
    else:
        if not valid_endpoint(topcatalog["endpoint"]):
            print(
                "Invalid endpoint. Endpoint must be a an accessible S3 "
                "(or equivalent) bucket link. It must start "
                'with "s3://" or "https://" and end in a terminating "/".'
            )

    if "egress" not in topcatalog:
        print(
            'No entry for "egress" provided! Egress should be a set of '
            "keywords defining permissions. Currently "
            "optional but will be required in the near future."
        )

    if "status" not in topcatalog:
        print(
            'No entry for "status" provided! Status is a return code and '
            "should communicate if a dataset is "
            "temporarily down or has other constraints. Default is "
            '"code: 1200, message: OK".'
        )

    if "contact" not in topcatalog:
        print(
            'No entry for "contact" provided! Contact should be the name '
            "and email address of the person to contact "
    ...
    for idx, catalog in enumerate(datasetcatalog):
        ...
        if "id" not in catalog:
            print(
                'No entry for "id" provided! Dataset ID is a ' "required catalog entry!"
            )
        else:
            if not valid_id(catalog["id"]):
                print(
                    "Invalid dataset ID. ID can only contain alphanumeric "
                    "characters, dashes, or underscores. No "
                    "spaces or any other characters."
                )

        if "index" not in catalog:
            print(
                'No entry for "index" provided! '
                "Dataset index is a required catalog entry!"
            )
        else:
            if not valid_index(catalog["index"]):
                print(
                    "Invalid index. Index must be a pointed to the "
                    "object directory and contain the dataset name. It "
                    'must start with "s3://" or "https://" and '
                    'end in a terminating "/".'
                )

        if "start" not in catalog:
            print(
                'No entry for "start" provided! '
                "Start date is a required catalog entry!"
            )
        else:
            if not valid_start(catalog["start"]):
                print(
                    "Invalid start date. Start date must be a date/time string "
                    "in Restricted ISO 8601 format: "
                    "XXXX-XX-XXTXXZ with at least the "
                    "year, month, day, and hour specified."
                )

        if "stop" not in catalog:
            print(
                'No entry for "stop" provided! '
                "Stop date is a required catalog entry!"
            )
        else:
            if not valid_stop(catalog["stop"]):
                print(
                    "Invalid stop date. Stop date must be a date/time string "
                    "in Restricted ISO 8601 format: "
                    "XXXX-XX-XXTXXZ with at least the "
                    "year, month, day, and hour specified."
                )
        ...
        if "modification" not in catalog:
            print(
                'No entry for "modification" provided! '
                "Modification date is a required catalog entry!"
            )
        else:
            if not valid_modification(catalog["modification"]):
                print(
                    "Invalid modification date. Modification date must be "
                    "a date/time string in Restricted ISO 8601 "
                    "format: XXXX-XX-XXTXXZ with at least the "
                    "year, month, day, and hour specified."
                )

        if "title" not in catalog:
            print(
                'No entry for "title" provided! '
                "Dataset title is a required catalog entry!"
            )

        if "indextype" not in catalog:
            print(
                'No entry for "indextype" provided! '
                "Index type is a required catalog entry!"
            )
        else:
            if not valid_indextype(catalog["indextype"]):
                print(
                    'Invalid entry for "indextype." Index type can be '
                    "either csv, csv-zip, or parquet."
                )

        if "filetype" not in catalog:
            print(
                'No entry for "filetype" provided! '
                "File type is a required catalog entry!"
            )
        else:
            if not valid_filetype(catalog["filetype"]):
                print(
                    'Invalid entry for "filetype." Current permitted types '
                    "are fits, csv, cdf, netcdf3, netcdf4, "
        ...
        if "creation" not in catalog:
            ...
            print(
                'Invalid entry for "creation." Creation date must be '
        ...
        if "expiration" not in catalog:
            ...
            print(
                'Invalid entry for "expiration." Expiration date must be '
                "a date/time string in ISO 8601 format: "
                "XXXX-XX-XXTXXZ with at least the "
                "year, month, day, and hour specified."
            )
        ...
        if "verified" not in catalog:
            ...
            print(
                'Invalid entry for "verified." Verified date must be '
                "a date/time string in ISO 8601 format: "
                "XXXX-XX-XXTXXZ with at least the "
                "year, month, day, and hour specified."
            )
        ...
        if "multiyear" not in catalog:
            ...
            print(
                'Invalid entry for "multiyear." Multiyear entry must '
