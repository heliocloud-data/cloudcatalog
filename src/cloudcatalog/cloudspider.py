        ...
        try:
            ...
            if collection != None:
                items = [
                    item
                    for item in items
                    if "collections" in item.keys()
                    and collection in item["collections"]
                ]
            if len(items) > 0:
...
def spider(spiderset=None, fr=None, noisy=True, create_manifest=False):
    ...
    for trio in spiderset:
        ...
        if icount % 3 == 1:
            print(f"   (checked {icount} of {totcount})")
    fout.close()
    if create_manifest:
        mout.close()
...
def spider_main():
    ...
    p.add_argument(
        ...
        default=False,
        help="(optional) dump entire catalog to manifest_spider.csv",
    )
    args = p.parse_args()
