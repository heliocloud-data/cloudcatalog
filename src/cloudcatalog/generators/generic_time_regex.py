...
def older_parse_filename_for_starttime(filename, pattern=None, prefix=""):
    ...
    if pattern is None:
        pattern = re.compile(
            r"^(.*?)_+" + prefix + r"(\d{4})(\d{2})(\d{2})(\d{2})?(\d{2})?(\d{2})?.*"
        )
    ...
    if match.groups()[5] is not None:
        minute = match.groups()[5]
    if match.groups()[6] is not None:
        second = match.groups()[6]
...
def call_gtr():
    ...
    with gzip.open("filelist.gz", "rt") as fin:
        ...
        ypattern4 = re.compile(
            r"^(.*?)_(\d{4})(\d{3})[Tt]?(\d{2})?(\d{2})?(\d{2})?[_-].*"
        )
